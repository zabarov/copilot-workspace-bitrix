...
- Правила и стандарты Bitrix: `rules/README.md` и все файлы в `rules/` — учитывай их перед генерацией ТЗ, архитектуры и кода.
...
- **40-tools-init** (`.github/agents/40-tools-init.agent.md`): определяет модуль в `local/modules/`, уточняет у пользователя контекст и готовит конфиги статических проверок (PHPStan, Deptrac, при запросе — PHPUnit/CS Fixer) под выбранный модуль, чтобы их можно было запускать без ручных правок путей.
