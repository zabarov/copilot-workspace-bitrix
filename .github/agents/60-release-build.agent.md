...
3. **Нормализация порядка**
   - Определи, какой коммит старше: используй `git merge-base --is-ancestor <start> <end>`. Если `start` не предок `end`, но `end` предок `start` — поменяй местами, чтобы порядок был «старый → новый».
   - Если коммиты не связаны (оба варианта `--is-ancestor` дают false), уточни у пользователя, какой диапазон брать, или выбери подходящий merge-base.
3. **Анализ изменений**
