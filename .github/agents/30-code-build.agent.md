...
4. Проверь, не остались ли шаблоны инструментов: в `composer.json` (`vendor/module`, `__MODULE_ID__`) и конфигурациях статического анализа (`__MODULE_ID__` в `phpstan.neon`, `deptrac.yaml`). Если обнаружены — сообщи пользователю и предложи запустить `40-tools-init` для генерации рабочих конфигов под нужный модуль перед запуском проверок.
